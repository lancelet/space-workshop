...
We will set up a Hohmann transfer, which is a way to transfer between two coplanar orbits \cite{battin1999}. To simplify the situation, we will assume that the source and destination orbits are circular. Analytical treatments of Hohmann transfer manoeuvres assume that an instantaneous impulse can be delivered to the spacecraft. In reality, this instantaneous impulse becomes a burn that is merely very short compared with the orbital periods involved.
...
\end{align}
Now consider the derivatives of these unit vectors with respect to the angle \gls{theta} (and notice that they have no dependence on \gls{rs}):
\begin{align}
...
\end{align}
As indicated, we will need the time derivatives of these basis vectors. Relative to the inertial system, they depend only on \gls{theta}, so the time derivatives can be expressed using the chain rule as follows:
\begin{align}
...
\end{align}
To describe motion in this coordinate system, we must have terms for position, velocity and acceleration. Position, \gls{r}, is given very simply by:
\begin{align}
  \gls{r} &= \gls{rs}\gls{er}
\end{align}
Velocity, \gls{v}, is then the time derivative, using the chain rule:
\begin{align}
...
\end{align}
Finally, acceleration, \gls{a}, is the time derivative of velocity:
\begin{align}
...
\section{Energetics of Transfer Orbits}

...
\begin{problem}[label=rk4shm]{Using the RK4 Integrator to Simulate Apollo Lunar Ascent Guidance.}
...
\chapter{Rocket Staging: Mass Budget, Specific Impulse and the Tsiolkovsky Rocket Equation}
...
Holt and Monk \cite{holt2009} provide some plots of typical propellant mass fractions for various launch systems. As a nominal figure we will choose \(\gls{zeta}=0.9\) for our simulations. This figure does not greatly impact the observations we will eventually make regarding the impact of staging, so a ``typical'' value is entirely adequate to illustrate the point.
...
\section{Simulating The Effect of Staging}
...
\begin{table}
  ...
  \begin{tabular}[htbp]{ l r l l }
    ...
    \bottomrule
  \end{tabular}
  \caption{Single-stage rocket parameters.}
  \label{tab:single-stage}
\end{table}

\begin{table}
  \centering
  \begin{tabular}[htbp]{ l r l l l }
    \toprule
    ...
    \bottomrule
  \end{tabular}
  \caption{Two-stage rocket parameters.}
  \label{tab:two-stage}
\end{table}

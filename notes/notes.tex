...
\glsxtrnewsymbol[description={Acceleration (\si{\m/\s/\s}).}]{a}{\ensuremath{\mathbf{a}}}
\glsxtrnewsymbol[description={Ellipse semi-major axis (\si{\m}).}]{as}{\ensuremath{a}}
\glsxtrnewsymbol[description={Ellipse semi-minor axis (\si{\m}).}]{bs}{\ensuremath{b}}
\glsxtrnewsymbol[description={Unit vector in the \(r\) direction.}]{er}{\ensuremath{\mathbf{e_r}}}
\glsxtrnewsymbol[description={Unit vector in the \(x\) direction.}]{ex}{\ensuremath{\mathbf{e_x}}}
\glsxtrnewsymbol[description={Unit vector in the \(y\) direction.}]{ey}{\ensuremath{\mathbf{e_y}}}
\glsxtrnewsymbol[description={Unit vector in the \(\theta\) direction.}]{etheta}{\ensuremath{\mathbf{e_{\theta}}}}
\glsxtrnewsymbol[description={Eccentricity.}]{e}{\ensuremath{e}}
\glsxtrnewsymbol[description={Total orbital energy (\si{J}).}]{E}{\ensuremath{E}}
\glsxtrnewsymbol[description={A function.}]{f}{\ensuremath{f}}
\glsxtrnewsymbol[description={Total force, scalar (\si{N}).}]{Fs}{\ensuremath{F}}
\glsxtrnewsymbol[description={Force due to gravity (\si{N}).}]{FG}{\ensuremath{\mathbf{F_G}}}
\glsxtrnewsymbol[description={Thrust force, scalar (\si{N}).}]{FTs}{\ensuremath{F_T}}
\glsxtrnewsymbol[description={Standard Earth gravity (\SI{9.80665}{m/s^2}).}]{g0}{\ensuremath{g_0}}
\glsxtrnewsymbol[description={Gravitational constant (\SI{6.67408E-11}{m^3 kg\per s^2}).}]{G}{\ensuremath{G}}
\glsxtrnewsymbol[description={Time step (\si{s}).}]{dt}{\ensuremath{h}}
\glsxtrnewsymbol[description={Specific angular momentum (\si{m^2\per\s}).}]{h}{\ensuremath{h_{\omega}}}
...
\glsxtrnewsymbol[description={Mass of dominant body (eg.\ a planet) (\si{kg}).}]{M}{\ensuremath{M}}
...
\glsxtrnewsymbol[description={Momentum (\si{kg m\per\s}).}]{p}{\ensuremath{\mathbf{p}}}
\glsxtrnewsymbol[description={Position (\si{m}).}]{r}{\ensuremath{\mathbf{r}}}
...
\glsxtrnewsymbol[description={Apoapsis radius (\si{m}).}]{ra}{\ensuremath{r_a}}
\glsxtrnewsymbol[description={Periapsis radius (\si{m}).}]{rp}{\ensuremath{r_p}}
\glsxtrnewsymbol[description={Inner radius of Hohmann transfer(\si{m}).}]{r1}{\ensuremath{r_1}}
\glsxtrnewsymbol[description={Outer radius of Hohmann transfer(\si{m}).}]{r2}{\ensuremath{r_2}}
...
\glsxtrnewsymbol[description={Velocity (\si{m/s}).}]{v}{\ensuremath{\mathbf{v}}}
\glsxtrnewsymbol[description={Velocity, scalar (\si{m/s}).}]{vs}{\ensuremath{v}}
\glsxtrnewsymbol[description={Apoapsis velocity, scalar (\si{m/s}).}]{va}{\ensuremath{v_a}}
\glsxtrnewsymbol[description={Periapsis velocity, scalar (\si{m/s}).}]{vp}{\ensuremath{v_p}}
...
\glsxtrnewsymbol[description={Velocity change for Hohmann transfer. (\si{m/s}).}]{deltav1}{\ensuremath{\Delta{}v_1}}
\glsxtrnewsymbol[description={Velocity change for Hohmann transfer. (\si{m/s}).}]{deltav2}{\ensuremath{\Delta{}v_2}}
\glsxtrnewsymbol[description={Radioactive decay constant (\si{\per\s}).}]{lambda}{\ensuremath{\lambda}}
\glsxtrnewsymbol[description={Standard gravitational parameter (\si{m^3\per s^2}).}]{mu}{\ensuremath{\mu}}
\glsxtrnewsymbol[description={Angle (radians).}]{theta}{\ensuremath{\theta}}
...
We will set up a Hohmann transfer, which is an elliptical transfer between two coplanar circular orbits \cite{battin1999,widnall2008}. Our analytical treatment of the Hohmann transfer manoeuvres will make the approximation that an instantaneous impulse can be delivered to the spacecraft, and its mass does not change, so that its motion obeys Kepler's laws at all times. In reality, this instantaneous impulse becomes a burn that is merely very short compared with the orbital periods involved, and the reaction mass used is small compared with the vehicle mass.

The analytical derivation in this chapter involves some mathematics that is more involved than previous chapters. Feel free to skip it if necessary (only the results are important for the numerical simulation). We provide it because a short yet complete treatment is somewhat difficult to find online.
...
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}
    % Main coordinate lines
    \draw[semithick,->] (-0.5,0) -- (4,0) node [right] {\(x\)};
    \draw[semithick,->] (0,-0.5) -- (0,4) node [left] {\(y\)};
    % ex and ey
    \draw[ultra thick,<->]
    (1,0) node [below] {\gls{ex}}
    -- (0,0)
    -- (0,1) node [left] {\gls{ey}};
    %\draw[ultra thick,->] (0,0) -- (1,0) node [below]{\gls{ex}};
    %\draw[ultra thick,->] (0,0) -- (0,1) node [left]{\gls{ey}};
    % point
    \fill (3,2) circle [radius=0.1];
    % er and etheta
    \begin{scope}[rotate=33.69,xshift=3.60555cm]
      \draw[ultra thick,->] (0,0) -- (1,0) node [right]{\gls{er}};
      \draw[ultra thick,->] (0,0) -- (0,1) node [left]{\gls{etheta}};
    \end{scope}
    % r
    \draw[thick] (0,0) -- (3,2) node [midway,above] {\gls{rs}};
    % theta
    \draw[thick,->,shorten >=1mm,shorten <=1mm] (2,0) arc [start angle=0,end angle=33.69,radius=2cm]
      node [midway,right] {\gls{theta}};
  \end{tikzpicture}
...
Under the sole action of gravity, which is a conservative force, the total energy of an object in orbit, \gls{E}, is constant, and is the sum of its kinetic and potential energy:
\begin{align}
  \gls{E} &= \frac{1}{2}\gls{m}\gls{vs}^2 - \frac{\gls{mu}\gls{m}}{\gls{rs}} \label{eq:orbit-energy}
\end{align}
in which \gls{vs} is the magnitude of the orbital velocity in this case. We can relate \gls{E} to the eccentricity of the orbit, \gls{e}, by computing \gls{E} specifically at periapsis (closest point of approach, where \(\gls{theta} = 0\)). At periapsis, \gls{rs} is given by:
\begin{align}
  \gls{rs} &= \frac{\gls{h}^2}{\gls{mu}(1 + \gls{e})} \label{eq:r-periapsis}
\end{align}
From Equations~\ref{eq:constant-sam} and \ref{eq:r-periapsis}, we have:
\begin{align}
  \gls{vs}^2 &= \frac{\gls{h}^2}{\gls{rs}^2} \\
  &= \frac{\gls{mu}(1 + \gls{e})}{\gls{rs}}
\end{align}
Substituting these into Equation~\ref{eq:orbit-energy} gives:
\begin{align}
  \gls{E} &= \frac{1}{2}\gls{m}\gls{vs}^2 - \frac{\gls{mu}\gls{m}}{\gls{rs}} \\
  &= \frac{1}{2}\frac{\gls{m}\gls{mu}}{\gls{rs}}(1+\gls{e})-\frac{\gls{m}\gls{mu}}{\gls{rs}} \\
  &= \frac{1}{2}\frac{\gls{m}\gls{mu}}{\gls{rs}}(\gls{e} - 1) \\
  &= \frac{1}{2}\gls{m}\gls{mu}\frac{\gls{mu}(\gls{e}-1)(\gls{e}+1)}{\gls{h}^2} \\
  &= \frac{\gls{m}\gls{mu}^2}{2\gls{h}^2}(\gls{e}^2-1)
\end{align}

\section{Hohmann Transfer Velocities}

From Equation~\ref{eq:kepler-solution}, we can write the 

\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}
    % Main ellipse
    \draw[ultra thick] (0,0) ellipse [x radius=3cm,y radius=2cm];
    % Focus
    \fill (2.235,0) circle [radius=0.1]
      node [above left] {Focus};
    % Background axes
    \draw[semithick] (-3.5,0) -- (4,0);
    \draw[semithick] (0,-3.8) -- (0,2.5);
    % Periapsis and apoapsis radii
    \draw[thick,<->] (2.235,-2.5) -- (3,-2.5) node [midway,below] {\gls{rp}};
    \draw[thick,<->] (-3,-2.5) -- (2.235,-2.5) node [midway,below] {\gls{ra}};
    % Semi-major axis
    \draw[thick,<->] (0,-3.3) -- (3,-3.3) node [midway,below] {\gls{as}};
    % Semi-minor axis
    \draw[thick,<->] (3.5,0) -- (3.5,-2) node [midway,right] {\gls{bs}};
    % Dashed dimensioning lines
    \draw[semithick,dashed] (3,1) -- (3,-3.8);
    \draw[semithick,dashed] (2.235,0.5) -- (2.235,-3);
    \draw[semithick,dashed] (-3,1) -- (-3,-3);
    \draw[semithick,dashed] (-1,-2) -- (4,-2);
  \end{tikzpicture}
  \caption{Ellipse geometric parameters.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    % Transfer orbit
    \draw[ultra thick] (1,0) arc [start angle=0,end angle=180,x radius=2cm,y radius=1.732cm];
    % Labels
    \draw (0,3) node [above] {\footnotesize{Outer orbit}};
    \draw (0,-1) node [below] {\footnotesize{Inner orbit}};
    \draw (-1,1.732) node [above] {\footnotesize{Transfer}};
    % Radii
    \draw[thick,->,shorten >=0.2mm] (0,0) -- (1,0) node [midway,below] {\gls{r1}};
    \draw[thick,->,shorten >=0.2mm] (0,0) -- (-3,0) node [midway,below] {\gls{r2}};
    % Delta-vs
    \draw[thick,->] (1,0) -- (1,1.2) node [right] {\gls{deltav1}};
    \draw[thick,->] (-3,0) -- (-3,-1.2) node [left] {\gls{deltav2}};
  \end{tikzpicture}

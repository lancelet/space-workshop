...
\glsxtrnewsymbol[description={Distance travelled (\si{\m}).}]{ds}{\ensuremath{d}}
...
\glsxtrnewsymbol[description={Thrust force (\si{N}).}]{FT}{\ensuremath{\mathbf{F_T}}}
...
\glsxtrnewsymbol[description={Allowable error in time (\si{\s}).}]{tepsilon}{\ensuremath{t_{\epsilon}}}
...
\glsxtrnewsymbol[description={Final velocity, scalar (\si{m/s}).}]{vsf}{\ensuremath{v_f}}
...
\begin{figure}[htbp]
  ...
  \caption{Moon-centred view of the burn and coasting phases. The radial (altitude) scale above the lunar surface is magnified by a factor of 20. The dashed line shows the target altitude. The target orbit is elliptical, as specified by the target velocity vector, which has a positive radial component.}
...
Figure~\ref{fig:ellipse-geometry} illustrates the geometry of an ellipse. The dimensions for its semi-minor axis, \gls{as}, and semi-major axis, \gls{bs}, are shown along with the radial distances of the periapsis (closest distance), \gls{rp}, and apoapsis (furthest distance), \gls{ra}, of an orbit with the more massive body at the focus.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \fill (0,0) circle [radius=0.1];
    % Circular orbits
    \draw[thick,dashed] (0,0) circle [radius=1];
    \draw[thick,dashed] (0,0) circle [radius=3];
    % Transfer orbit
    \draw[ultra thick,red] (1,0) arc [start angle=0,end angle=15,x radius=2cm,y radius=1.732cm];
    \draw[ultra thick,gray] (0.932,0.448) arc [start angle=15,end angle=180,x radius=2cm,y radius=1.732cm];
    \draw[ultra thick,red] (-3,0) arc [start angle=180,end angle=195,radius=3cm];
    % Horizontal line of apses
    \draw[semithick] (-3.5,0) -- (3.5,0);
    % Angle
    \fill (0.732,0.866) circle [radius=0.075];
    \draw[semithick] (0,0) -- (2.186,2.598);
    \draw[semithick,->,shorten >=1mm,shorten <=1mm] (2,0) arc [start angle=0,end angle=50,radius=2] node [midway,right] {\gls{theta}};
    % Labels
    \draw[red] (1,0) node [below right] {\footnotesize{First burn}};
    \draw[red] (-3,0) node [below left] {\footnotesize{Second burn}};
    \draw (-1,1.732) node [above] {\footnotesize{Coast}};
  \end{tikzpicture}
...
\begin{figure}[htbp]
  \resizebox{\textwidth}{!}{\input{fig/vertical-throw}}
...
\begin{figure}[htbp]
  \centering
  \resizebox{0.5\textwidth}{!}{\input{fig/hohmann-high-impulse}}
...
\begin{figure}[htbp]
  \centering
  \resizebox{0.5\textwidth}{!}{\input{fig/hohmann-low-impulse}}
...
\begin{figure}[htbp]
  \centering
  \resizebox{0.5\textwidth}{!}{\input{fig/hohmann-ultra-low-impulse}}
